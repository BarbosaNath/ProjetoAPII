...
def generate_layout(first_screen):
    ...
    for module in mods:
        layout[0] += [Column([[sg.VerticalSeparator(), Column(show_images(module), s=(600, 600), scrollable=True, vertical_scroll_only=True)]], key=f"images_{module}", s=(607,606), visible=False)]
    ...
    # ===============================================================
        ...
        if event[0] == 'modules_reset_screen':
            ...
            try:
                ...
                sg.popup_no_buttons(f"Selecione ao menos uma imagem!", background_color = "red", auto_close = True, auto_close_duration = 1, no_titlebar = True, keep_on_top = True)
            ...
            if values["file_image"] == "":
                sg.popup_no_buttons(f"Imagem não pode ficar vazia!", background_color = "red", auto_close = True, auto_close_duration = 1, no_titlebar = True, keep_on_top = True)
                continue
            if values["product_code"] == "":
                sg.popup_no_buttons(f"Referencia: não pode ficar vazio!", background_color = "red", auto_close = True, auto_close_duration = 1, no_titlebar = True, keep_on_top = True)
                continue
            ...
            try:
                ...
                sg.popup_no_buttons(f"Referência já existe!", background_color = "red", auto_close = True, auto_close_duration = 1, no_titlebar = True, keep_on_top = True)
                continue
